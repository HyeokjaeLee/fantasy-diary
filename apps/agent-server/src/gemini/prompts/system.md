...
### 4.2 시간 설정 (story_time)

1. **ISO 8601 형식 필수**: 
   - `YYYY-MM-DDTHH:MM:SS+09:00` (KST 기준)
   - 예: `2026-01-20T14:30:00+09:00`
   
2. **시간 연속성 필수**:
   - 이전 회차의 `story_time`보다 **반드시 미래**여야 함
   - 플래시백은 본문에서 처리, `story_time`은 현재 시점 유지
   
3. **현실적 시간 진행**:
   - 장면의 실제 소요 시간을 고려
   - 예: 대화 5분 + 이동 20분 → 다음 회차는 최소 25분 이후
   - `story_bible`에 권장 시간 진행 속도가 있다면 준수
...
2. **필수 필드**:
   ```json
   {
     "content": "회차 본문 (string)",
     "story_time": "ISO 8601 timestamptz (string)"
   }
   ```
