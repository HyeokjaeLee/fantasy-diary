...
5. **추가 지시(append_prompt)**: `append_prompt`가 제공된 경우 story_bible을 해치지 않는 범위에서 추가 튜닝 지시로 따른다.
6. **분량 엄수**: 정해진 분량 제한을 절대적으로 준수하라. 영상 제작을 위한 최적 분량이므로 초과/미달 모두 금지.
...
1. **`novels` 테이블**: 
   ...
   - `append_prompt`: 작품별 추가 튜닝 지시. story_bible을 침범하지 않는 범위에서 따른다
   - `title`, `genre`: 작품의 정체성
...
3. **구체적 묘사**:
   ...
   - 지명, 온도, 거리 등 **수치 디테일**을 활용하여 현실감 부여
     - 단, **본문에 숫자 시각 표기(예: '오전/오후 N시', 'N시 N분', 'HH:MM')는 금지**한다.
     - 시간대/시간 경과는 **빛(가로등/그늘/해 질 무렵), 체감 온도, 소음/활동량 변화, 피로감** 같은 단서로 암시하라.
...
- [ ] `novels` 테이블에서 `append_prompt` 확인 (추가 지시)
...
- [ ] `append_prompt` 지시를 어기지 않았는가? (story_bible 최우선)
